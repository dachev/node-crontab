...
function CronTab(u, cb) {
    ...
    // Create a new cron with a command, comment and date. Returns the new CronItem object.
    this.create = function(command) {
        var comment = (typeof arguments[1] == 'string') ? arguments[1] : null,
            item    = makeItem(null, command, comment);
        ...
        var date = (arguments[1] instanceof Date) ?
            arguments[1] : (arguments[2] instanceof Date) ?
                arguments[2] :
                null;
        
        if (date && date instanceof Date) {
            item.minute().on(date.getMinutes());
            item.hour().on(date.getHours());
            item.dom().on(date.getDate());
            item.month().on(date.getMonth()+1);
        }
...
function CronSlot(name, min, max, enum, value) {
    ...
    this.every = function(n) {
        try {
            var range = new CronRange(self, '*/' + parseInt(n));
            parts = [range];
        }
        catch (e) {}
    }
    ...
    this.during = function(from, to) {
        try {
            var range = new CronRange(self, from + '-' + to);
            parts.push(range);
        ...
        if (value) {
            ...
            for (var i = 0; i < tokens.length; i++) {
                ...
                if (token.indexOf('/') > 0 || token.indexOf('-') > 0 || token == '*') {
                    var range = new CronRange(self, token);
                    parts.push(range);
                }
                else {
