...
function CronTab(u, cb) {
    ...
    /**
     * Returns a list of jobs using the specified command.
     * 
     * Examples:
     *     new CronTab(function(err, tab) {
     *         if (err) { console.log(err); process.exit(1); }
    ...
    this.findCommand = function(command) {
        ...
        for (var i = 0; i < jobs.length; i++) {
            ...
            if (job.getCommand().match(command)) {
                results.push(job);
            }
        }
    ...
    /**
     * Removes all jobs using the specified command.
     * 
     * Examples:
     *     new CronTab(function(err, tab) {
     *         if (err) { console.log(err); process.exit(1); }
     ...
     * @param {String} __[meta]__
     * 
     * @api private
     */
    function makeJob(line, command, meta) {
        try {
            return new CronJob(line, command, meta);
        } catch(e) {}
...
function CronJob(line, c, m) {
    ...
    /**
     * Returns the command object associated with this cron job.
     * 
     * Examples:
     *     new CronTab(function(err, tab) {
     *         if (err) { console.log(err); process.exit(1); }
    ...
    function setSlots(tokens) {
        ...
        for (var i = 0; i < 5; i++) {
            var info  = SINFO[i],
                ...
                max   = info.max,
                enum  = info.enum,
                slot  = new TimeSlot(name, min, max, enum, value);
 ...
 * more time ranges coresponding to the comma separated list in the cron sytax (e.g. _* / 4_, _10_, 5-15/2).
 * 
 * Examples:
 ...
 * @param {Object|null} __enum__ an object enumerating all possible values
...
function CronCommand(line) {
     ...
     *         for (var i = 0; i < jobs.length; i++) {
     *             // false
     *             console.log(jobs[i].getCommand().match('pwd'));
     *         }
     *     });
     *
     * @param {String} __line__
     ...
     *         var jobs = tab.findCommand('ls -l /');
     *         for (var i = 0; i < jobs.length; i++) {
     *             console.log(jobs[i].getCommand().toString());
     *         }
     *     });
     *
     * @return {String}
